...
auto S71GL064A08::load_data(std::span<const u8> new_data) -> bool
{
    ...
    {
        std::printf("[S71GL064A08] tried loading bad save data: %zu\n", new_data.size());
        return false;
    }
}

auto S71GL064A08::get_data() const -> std::span<const u8>
{
    return this->flash;
}

template<typename T>
auto S71GL064A08::read_flash_internal(u32 addr) const -> T
{
...
template<typename T>
void S71GL064A08::write_flash_internal(u32 addr, T value)
{
    switch (this->state)
    {
        case State::READY:
            ...
            {
                // std::printf("[S71GL064A08] invalid ready addr: 0x%04X value: 0x%04X\n", addr, value);
                // assert(0);
            }
            // set bank
            else if (this->command == Command::SetMemoryBank)
            {
                ...
                if (value > max_bank)
                {
                    std::printf("[S71GL064A08] invalid bank set in flash64\n");
                    assert(0);
                    this-> bank = 0;
                }
                else
            ...
            else if (this->command == Command::UnknownC0)
            {
                ...
                {
                    std::printf("[S71GL064A08] bad value: 0x%08X v: 0x%04X\n", addr, value);
                }
        ...
        case State::CMD1:
            ...
            {
                std::printf("[S71GL064A08] invalid cmd1 write to 0x%08X value: 0x%02X\n", addr, value);
                this->state = State::READY;
                assert(0);
            }
            break;
        ...
        case State::CMD2:
            if ((addr & 0xFFFF) == ADDR_5555)
            {
                ...
                switch (this->command)
                {
                    ...
                    case Command::EraseAll:
                        std::printf("[S71GL064A08] eraseAll\n");
                        std::memset(this->flash, 0xFF, sizeof(this->flash));
                        break;

                    default:
                        std::printf("[S71GL064A08] unknown command value: 0x%02X\n", value);
                        break;
                }
            }
            // sector erase
            ...
            {
                std::printf("[S71GL064A08] sector erase addr: 0x%08X\n", addr);
            ...
            {
                assert(!"[S71GL064A08] invalid 3 write command");
            }
            this->state = State::READY;
            break;
        ...
        case State::BUFFER:
            if (this->buffer_count)
            {
                ...
                if (addr < 0x200000)
                {
                    std::printf("[S71GL064A08-BUFFER] kernel writing to addr: 0x%08X\n", addr);
                    assert(0);
                }
...
void S71GL064A08::write_flash_data(u32 addr, const void* data, u32 size)
{
...
void S71GL064A08::read_flash_data(u32 addr, void* data, u32 size) const
{
...
template<typename T>
auto S71GL064A08::read_ram_internal(u32 addr) const -> T
{
    if constexpr(std::is_same<T, u8>())
    {
...
template<typename T>
void S71GL064A08::write_ram_internal(u32 addr, T value)
{
    if constexpr(std::is_same<T, u8>())
    {
...
template<> auto S71GL064A08::read_flash<u8>(u32 addr) const -> u8
{
    return read_flash_internal<u8>(addr);
}
template<> auto S71GL064A08::read_flash<u16>(u32 addr) const -> u16
{
    return read_flash_internal<u16>(addr);
}
template<> auto S71GL064A08::read_flash<u32>(u32 addr) const -> u32
{
...
template<> auto S71GL064A08::read_ram<u8>(u32 addr) const -> u8
{
    return read_ram_internal<u8>(addr);
}
template<> auto S71GL064A08::read_ram<u16>(u32 addr) const -> u16
{
    return read_ram_internal<u16>(addr);
}
template<> auto S71GL064A08::read_ram<u32>(u32 addr) const -> u32
{
